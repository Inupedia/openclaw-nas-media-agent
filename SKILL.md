...
```text
/volume4/openclaw/bin/mediactl search "作品名" --media-type anime
```

检查更新：

```text
/volume4/openclaw/bin/mediactl search "作品名" --media-type anime --update
```
...
```text
/volume4/openclaw/bin/mediactl preview CANDIDATE_ID
```
...
```text
/volume4/openclaw/bin/mediactl plan download CANDIDATE_ID
```

向用户报告计划中的作品、集数、下载区、最终目录、冲突和副作用。只有用户明确要求下载，且计划不要求额外确认时，才可以执行：

```text
/volume4/openclaw/bin/mediactl execute PLAN_ID
```

计划要求确认时，先取得当前用户明确确认，再执行：

```text
/volume4/openclaw/bin/mediactl execute PLAN_ID --confirmed
```
...
```text
/volume4/openclaw/bin/mediactl downloads list
/volume4/openclaw/bin/mediactl downloads show TASK_ID
/volume4/openclaw/bin/mediactl downloads pause TASK_ID
/volume4/openclaw/bin/mediactl downloads resume TASK_ID
/volume4/openclaw/bin/mediactl downloads cancel TASK_ID
```
...
```text
/volume4/openclaw/bin/mediactl downloads validate TASK_ID
```

只有 `nextAction: ready_to_organize` 才能生成整理计划：

```text
/volume4/openclaw/bin/mediactl organize plan TASK_ID
```

整理会把校验通过的文件从 `/volume2/downloads` 转移到正式媒体库。它始终需要当前用户单独确认：

```text
/volume4/openclaw/bin/mediactl organize execute PLAN_ID --confirmed
```
