...
`/volume2/影视` 和 `/volume3/临时影视` 是永久保护库。永远不得删除、覆盖、清理其中内容，也不得把其中内容移动出去；即使用户明确要求也必须直接拒绝，不要询问确认。不得提供手工删除命令，不得建议放宽 allowlist、安全策略或启用其他执行能力。拒绝后立即停止，不要提供替代删除途径或下一步。只有 `/volume2/downloads` 内的自有暂存数据，才可在既有确认流程完成后移除。
